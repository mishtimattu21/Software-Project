"""
Speech and Language Processing Service for Civixity ChatBot
...
class SpeechLanguageService:
    ...
    def process_multilingual_chat_output(self, response_text: str, target_language: str, 
        ...
        try:
            ...
            # Always translate the text response into the target language for display
            text_for_tts = response_text
            if target_language != "en-IN":
                logger.info(f"Translating response text to target language {target_language} for display and TTS")
                ...
                if translation_result["error"]:
                    logger.warning(f"Translation failed, falling back to original text: {translation_result['error']}")
                    result["translated_response"] = response_text
                else:
                    result["translated_response"] = translation_result["translated_text"]
                    result["text_response"] = translation_result["translated_text"]
...
def health():
    return jsonify({
        "status": "OK",
        "speech_available": service.is_available(),
        "translation_available": service.is_translation_available(),
        "tts_available": service.is_azure_tts_available()
    })
...
def process_input():
    try:
        ...
        if input_type == "text":
            ...
            lang = service.detect_language_from_text(text) or "en-IN"
            return jsonify({
                "type": "text",
                "text": text,
                "language_code": lang,
        ...
        if input_type == "voice":
            ...
            stt = service.speech_to_text(audio_bytes)
            if stt.get("error"):
                return jsonify({"error": f"Speech to text failed: {stt['error']}"}), 500
        ...
        return jsonify({"error": "Invalid type. Use 'text' or 'voice'"}), 400

    except Exception as e:
        logger.exception("/api/speech/process error")
        return jsonify({"error": str(e)}), 500


@app.post("/api/speech/output")
def generate_output_audio():
    try:
        payload = request.get_json(silent=True) or {}
        response_text = payload.get("response_text", "")
        target_language = payload.get("target_language") or "en-IN"
        input_type = payload.get("input_type", "text")  # original user input type

        if not response_text:
            return jsonify({"error": "response_text is required"}), 400

        processed = service.process_multilingual_chat_output(
        ...
        audio_b64 = None
        if processed.get("audio_response"):
            try:
                audio_b64 = base64.b64encode(processed["audio_response"]).decode("utf-8")
            except Exception:
                audio_b64 = None

        return jsonify({
            "text_response": processed.get("text_response"),
            "translated_response": processed.get("translated_response"),
            "audio_base64": audio_b64,  # present when input_type == 'voice' and TTS succeeded
            "target_language": processed.get("target_language"),
            "error": processed.get("error")
        })

    except Exception as e:
        logger.exception("/api/speech/output error")
        return jsonify({"error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("SPEECH_SERVICE_PORT", "5001"))
    debug = os.getenv("SPEECH_SERVICE_DEBUG", "false").lower() == "true"
